# FinFocus AWS Public Plugin Documentation

Welcome to the comprehensive documentation for the FinFocus AWS Public Plugin.
This plugin provides accurate AWS infrastructure cost estimation using embedded
public pricing data.

## Table of Contents

### Getting Started

- [README](../README.md) - Quick start guide and overview
- [Installation & Setup](../README.md#installation--setup) - Build and installation instructions
- [Quick Start](../README.md#quick-start) - Basic usage examples

### User Guides

- [Supported Resources](../README.md#supported-resources) - Complete list of supported AWS services
- [Cost Estimation](../README.md#cost-estimation) - How costs are calculated
- [Carbon Estimation](../README.md#carbon-estimation-ec2-only) - Environmental impact tracking
- [Troubleshooting](../README.md#troubleshooting) - Common issues and solutions

### API Documentation

- [API Reference](api.md) - Complete gRPC API documentation
- [Resource Types](api.md#resource-types) - Detailed resource specifications
- [Error Codes](api.md#error-codes) - Error handling reference
- [Transport Protocol](api.md#transport) - gRPC communication details

### Developer Documentation

- [Architecture](../README.md#architecture) - System design and components
- [Building & Testing](../README.md#development) - Development workflow
- [Contributing](CONTRIBUTING.md) - How to contribute
- [Release Process](../RELEASING.md) - Version management and releases
- [Distribution](distribution.md) - Build and release configuration

### Regional Management

- [Region Management](region-management.md) - Managing AWS regions and builds
- [Adding New Regions](region-management.md#adding-new-regions) - Step-by-step guide

### Internal Documentation

- [AGENTS](../AGENTS.md) - Development guidelines for AI assistants
- [CLAUDE](../CLAUDE.md) - Claude Code development context
- [CHANGELOG](../CHANGELOG.md) - Version history and changes
- [TROUBLESHOOTING](../TROUBLESHOOTING.md) - Advanced troubleshooting guide

## Quick Links

- 🏠 [Project Homepage](https://github.com/rshade/finfocus-plugin-aws-public)
- 📚 [FinFocus Core](https://github.com/rshade/finfocus-core)
- 🔧 [Protocol Specification](https://github.com/rshade/finfocus-spec)
- 🌍 [Cloud Carbon Footprint](https://www.cloudcarbonfootprint.org/)

## Support

- 📖 [Full Documentation Index](https://github.com/rshade/finfocus-plugin-aws-public/tree/main/docs)
- 🐛 [Report Issues](https://github.com/rshade/finfocus-plugin-aws-public/issues)
- 💬 [Discussions](https://github.com/rshade/finfocus-plugin-aws-public/discussions)

---

This documentation is automatically updated with each release. Last updated: v0.0.15
