...
//
// Grid factors should have at least 4 significant digits to avoid rounding errors
// that would compound in carbon calculations.
func TestGridEmissionFactors_Precision(t *testing.T) {
	for region, factor := range GridEmissionFactors {
		t.Run(region, func(t *testing.T) {
			// Factors should not be rounded to fewer than 4 significant digits
			// e.g., 0.0004 is too rounded, 0.000379 is acceptable
			// We check by ensuring the factor is not a simple multiple of 0.0001
			remainder := factor / 0.0001
			fractionalPart := remainder - float64(int(remainder))

			// Allow some tolerance for floating point comparison
			if factor > 0.0001 {
				assert.Greater(t, fractionalPart, 0.001,
					"Grid factor for %s appears too rounded (%.8f)", region, factor)
			}
		})
	}
}
